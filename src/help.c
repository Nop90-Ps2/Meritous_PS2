...
#ifdef PS2
	fp = fopen("cdrom0:\\DATA\\D\\HELPFILE.TXT", "r");
#else
...
#ifdef PS2
			if (strcmp(linkfollow, hlp->s[i]->identifier) == -13) {
#else
...
int MoveCursor()
{
	...
	while (SDL_PollEvent(&ev)) {
		...
		{
			if (ev.jbutton.button == 1) // X Button -> Select
			{
				my_link = 1;
			}
			else if (ev.jbutton.button == 0) // SQUARE Button -> Select
			{
				my_link = 1;
			}
			else if (ev.jbutton.button == 5) // SELECT Button -> Exit Help
			{
				return 0;
			}
			else if (ev.jbutton.button == 2) // O Button -> Exit Help
			{
				return 0;
			}
			else if (ev.jbutton.button == 4) // START Button -> Exit Help
			{
				return 0;
			}
			else if (ev.jbutton.button == 3) // TRIANGLE Button -> Exit Help 
			{
				return 0;
			}
		}
...
void freeHelp(void)
{
	...
	if(hlp!=NULL)
	{
		for(i=0; i<	hlp->sections; i++)
		{
			for(j=0;j<	hlp->s[i]->lines;j++)
				free(hlp->s[i]->l[j]);
			free(hlp->s[i]->identifier);
			free(hlp->s[i]);
		}
		free(hlp);
	}
