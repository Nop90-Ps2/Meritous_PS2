...
SDL_Color ending_pal[256];
	...
	SDL_Surface *theend[2] = {NULL};
	SDL_Surface *circ = NULL;
#endif
...
#ifdef PS2
	Paint(0, 0, 22, 27, "cdrom0:\\DATA\\D\\CSTREAM.LOC");
#else
...
void DrawCredits()
{
	...
	if (fin == NULL) {
#ifdef PS2
		fin = IMG_Load("cdrom0:\\DATA\\I2\\FIN.PNG");
#else
		fin = IMG_Load("dat/i/fin.png");
#endif
		
#ifdef PS2
		theend[0] = IMG_Load("cdrom0:\\DATA\\I2\\THEEND.PNG");
#else
		theend[0] = IMG_Load("dat/i/theend.png");
#endif
#ifdef PS2
		theend[0] = IMG_Load("cdrom0:\\DATA\\I2\\TRUE_END.PNG");
#else
...
void DrawCircuitFlash(int t, int method)
{
	...
	if (circ == NULL) {
#ifdef PS2
		circ = IMG_Load("cdrom0:\\DATA\\I2\\CIRCUITS.PG");
#else
...
#ifdef PS2
void freeEnding(void)
{
    if(circ != NULL)
        SDL_FreeSurface(circ);
    if(fin != NULL)
        SDL_FreeSurface(fin);
    if(theend[0] != NULL)
        SDL_FreeSurface(theend[0]);
    if(theend[1] != NULL)
        SDL_FreeSurface(theend[1]);
    if(glitter != NULL)
        SDL_FreeSurface(glitter);
    if(streamspr != NULL)
        SDL_FreeSurface(streamspr);
}
#endif
