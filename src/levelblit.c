...
/*			if (!strcasecmp(argv[i], "record")) {
...
#ifdef PS2
	PS2_init();
	SDL_Init(SDL_INIT_VIDEO | SDL_INIT_AUDIO | SDL_INIT_TIMER | SDL_INIT_JOYSTICK); // don't forget SDL_INIT_TMER on PS2 if you want audio
	joystick=SDL_JoystickOpen(0);
	
	PS2_load_MC(); // Call this after setting the joystick or the program will hung  
	asceai = IMG_Load("cdrom0:\\DATA\\I2\\ASCEAI.PNG");
#else
...
#ifdef PS2
	screen = SDL_SetVideoMode(SCREEN_W, SCREEN_H, 8, SDL_HWSURFACE | SDL_FULLSCREEN);
#else
	screen = SDL_SetVideoMode(SCREEN_W, SCREEN_H, 8, SDL_SWSURFACE | (SDL_FULLSCREEN * fullscreen));
#endif
	
#ifdef PS2
	wm_mask_file = fopen("cdrom0:\\DATA\\D\\BITMASK.DAT", "rb");
#else
	...
	SetGreyscalePalette();
#ifdef PS2
	//since we preload all the musics and sfxs to avoid lags, better say the user to wait
	memset(screen->pixels, 0, 640*480);
	DrawRect(200, 217, 240, 50, 80);
	DrawRect(202, 219, 236, 46, 20);
	draw_text(232, 228, "Loading... please wait", 255);
	VideoUpdate();
	InitAudio(); //movde init audio fter text and palette init
#endif	
	// asceai logo
...
#ifdef PS2
		title = IMG_Load("cdrom0:\\DATA\\I2\\TITLE.PNG");
		title_pr = IMG_Load("cdrom0:\\DATA\\I2\\TITLE.PNG");
...
#ifdef PS2
				DungeonPlay("mc0:/MERITOUS/SaveFile.sav");
...
void DrawMeter(int x, int y, int n)
{
	...
	if (meter == NULL) {
#ifdef PS2
		meter = IMG_Load("cdrom0:\\DATA\\I\\METER.PNG");
#else
			...
			if (player_room == agate_knife_loc) {
				{
					...
					if (agate_knife == NULL) {
#ifdef PS2
						agate_knife = IMG_Load("cdrom0:\\DATA\\I\\AGATE.PNG");
#else
...
#ifdef PS2
			draw_text(80, 11-6, "Use the movement keys to locate a checkpoint. Press SQUARE to", 240);
			draw_text(52, 11+6, "teleport to this checkpoint. Press CIRCLE or START once you are done.", 240);
#else
...
#ifdef PS2
			draw_text((640 - 23 * 8) / 2, (480 - 8) / 2 + 4, "Press SQUARE to confirm.", 255);
#else
...
void HandleEvents()
{
	...
	enter_pressed = 0;
		while (SDL_PollEvent(&event)) {
            ...
            {
				if (event.jbutton.button == 1) // X Button -> Select / Fire
				{
					key_held[K_SP] = 1;
				}
				else if (event.jbutton.button == 0) // SQUARE Button -> Activate
				{
					enter_pressed = 1;
				}
				else if (event.jbutton.button == 5) // SELECT Button -> Exit
				{
					if (map_enabled) {
						map_enabled = 0;
						game_paused = 0;
				...
				else if (event.jbutton.button == 2) // O Button -> Map
				{
					if (tele_select) {
						map_enabled = 0;
						game_paused = 0;
				...
				else if (event.jbutton.button == 4) // START Button -> Help
				{
						CancelVoluntaryExit();
						ShowHelp();
				}
				else if (event.jbutton.button == 3) // TRIANGLE Button -> Pause 
				{
						game_paused ^= 1;
						CancelVoluntaryExit();
				}
            ...
            {
				if (event.jbutton.button == 1) // X Button
				{
					key_held[K_SP] = 0;
				}
			}
...
/*
            if (event.type == SDL_JOYAXISMOTION)
            {
                ...
                {
                    if (event.jaxis.value < -MAX_JOY_MOTION)
                    {
						key_held[K_DN] = 0;
						key_held[K_UP] = 1;
                    }
                    else if (event.jaxis.value > MAX_JOY_MOTION)
                    {
						key_held[K_DN] = 1;
						key_held[K_UP] = 0;
                    }
...
void DrawLevel(int off_x, int off_y, int hide_not_visited, int fog_of_war)
{
	...
	if (tiles == NULL) {
#ifdef PS2
		tiles = IMG_Load("cdrom0:\\DATA\\I\\TILESET.PNG");
		fog = IMG_Load("cdrom0:\\DATA\\I\\TILESET.PNG");
#else				
...
void DrawPlayer(int x, int y, int pl_dir, int pl_frm)
{
	...
	if (playersprite == NULL) {
#ifdef PS2
		playersprite = IMG_Load("cdrom0:\\DATA\\I\\PLAYER.PNG");
#else
...
void text_init()
{
	...
	int chr, x, y;
#ifdef PS2
	font_data_file = fopen("cdrom0:\\DATA\\D\\FONT.DAT", "rb");
#else
...
void SpecialTile(int x, int y)
{
	...
	switch (tile) {
...
#ifdef PS2
				sprintf(message, "Press SQUARE to teleport between checkpoints.");
#else
		...
		case 26:
#ifdef PS2
			sprintf(message, "Press SQUARE to open the storage chest");
#else
...
#ifdef PS2
				sprintf(message, "Press SQUARE to upgrade shields (%d crystals)", UpgradePrice(0));
#else
...
#ifdef PS2
				sprintf(message, "Press SQUARE to upgrade circuit charge (%d crystals)", UpgradePrice(1));
#else
...
#ifdef PS2
				sprintf(message, "Press SQUARE to upgrade circuit refill (%d crystals)", UpgradePrice(2));
#else
		...
		case 31:
#ifdef PS2
			sprintf(message, "Press SQUARE to record your progress");
#else
...
#ifdef PS2
				sprintf(message, "Press SQUARE to activate the crystal device");
#else
		...
		default:
			if (first_game) {
				if (otext < 60) {
#ifdef PS2
					sprintf(message, "Press START to read the help file");
#else
...
void DrawArtifacts()
{
	...
	if (artifact_spr == NULL) {
#ifdef PS2
		artifact_spr = IMG_Load("cdrom0:\\DATA\\I\\ARTIFACT.PNG");
#else
