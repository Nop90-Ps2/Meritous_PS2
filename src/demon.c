...
#ifdef PS2
	SDL_Surface *teleflash = NULL;
	SDL_Surface *d_star_small = NULL;
	SDL_Surface *d_star_big = NULL;
	SDL_Surface *d_star_ls = NULL;
	SDL_Surface *d_sprite = NULL;
#endif
// enemy
...
void KillEnemy(struct enemy *t)
{
	...
	if ((ct - lastkill) > 100) {
#ifdef PS2
		SND_Pos(ENEMYH, 128, PlayerDist(t->x, t->y));
#else
...
void DrawEnemy(struct enemy *e, SDL_Surface *scr)
{
	...
	if (e->teleport_v < 24) {
		if (teleflash == NULL) {
#ifdef PS2
			teleflash = IMG_Load("cdrom0:\\DATA\\I\\TFLASH.PNG");
#else
...
void DrawBullet(struct bullet *b)
{
	...
	if (b->img == 1) {
		if (d_star_big == NULL) {
#ifdef PS2
			d_star_big = IMG_Load("cdrom0:\\DATA\\I\\STAR1.PNG");
#else
...
#ifdef PS2
			d_star_small = IMG_Load("cdrom0:\\DATA\\I\\STAR2.PNG");
#else
	...
	if (b->img == 6) {
		if (d_star_ls == NULL) {
#ifdef PS2
			d_star_ls = IMG_Load("cdrom0:\\DATA\\I\\STAR3.PNG");
#else
...
void DrawGem(struct diamond *g)
{
	...
	if (d_sprite == NULL) {
#ifdef PS2
		d_sprite = IMG_Load("cdrom0:\\DATA\\I\\GEM.PNG");
#else
...
{
    if(d_sprite != NULL)
        SDL_FreeSurface(d_sprite);

    if(reticle != NULL)
        SDL_FreeSurface(reticle);

    if(inrange != NULL)
        SDL_FreeSurface(inrange);

    if(invis_enemy != NULL)
        SDL_FreeSurface(invis_enemy);

    if(teleflash != NULL)
        SDL_FreeSurface(teleflash);

    if(d_star_big != NULL)
        SDL_FreeSurface(d_star_big);

    if(d_star_small != NULL)
        SDL_FreeSurface(d_star_small);

    if(d_star_ls != NULL)
        SDL_FreeSurface(d_star_ls);
