...
int percent_required[4] = {15, 40, 60, 75};
	...
	SDL_Surface *boss_spr0 = NULL;
	SDL_Surface *boss_spr1 = NULL;
	SDL_Surface *boss_spr2 = NULL;
	SDL_Surface *boss_spr3 = NULL;
...
void Curse()
{
	...
	rooms[0].room_type = 2;
#ifdef PS2
	Paint(rooms[0].x+1, rooms[0].y+1, rooms[0].w-2, rooms[0].h-2, "cdrom0:\\DATA\\D\\FBOSSROO.LOC");
#else
...
#ifdef PS2
		boss_hp_icon = IMG_Load("cdrom0:\\DATA\\I1\\BOSS.PNG");
#else
...
#ifdef PS2
			SND_Pos(ENEMYH, 128, dist(x, y, boss_x, boss_y) / 4);
#else
...
#ifdef PS2
			if (boss_spr0 == NULL) {
				boss_spr0 = IMG_Load("cdrom0:\\DATA\\I1\\BOSS1.PNG");
				SDL_SetColorKey(boss_spr0, SDL_SRCCOLORKEY | SDL_RLEACCEL, 255);
			}
			boss_spr = boss_spr0;	
#else
			if (boss_spr == NULL) {
...
#ifdef PS2
			if (boss_spr1 == NULL) {
				boss_spr1 = IMG_Load("cdrom0:\\DATA\\I1\\BOSS2.PNG");
				SDL_SetColorKey(boss_spr1, SDL_SRCCOLORKEY | SDL_RLEACCEL, 255);
			}
			boss_spr = boss_spr1;	
#else
			if (boss_spr == NULL) {
...
#ifdef PS2
			if (boss_spr2 == NULL) {
				boss_spr2 = IMG_Load("cdrom0:\\DATA\\I1\\BOSS3.PNG");
				SDL_SetColorKey(boss_spr2, SDL_SRCCOLORKEY | SDL_RLEACCEL, 0);
			}
			boss_spr = boss_spr2;	
#else
			if (boss_spr == NULL) {
...
#ifdef PS2
			if (boss_spr3 == NULL) {
				boss_spr3 = IMG_Load("cdrom0:\\DATA\\I1\\BOSS4.PNG");
				SDL_SetColorKey(boss_spr3, SDL_SRCCOLORKEY | SDL_RLEACCEL, 0);
			}
			boss_spr = boss_spr3;	
#else
			if (boss_spr == NULL) {
				...
				if (t_timer < 220) {
					...
					if (boss_fight_mode < 23) {
						...
						if (endpics[0] == NULL) {
							if (training) {
#ifdef PS2
								endpics[0] = IMG_Load("cdrom0:\\DATA\\I2\\WUSS_END.PNG");
#else
...
void freeBoss(void)
{
    ...
    if(boss_spr0 != NULL)
        SDL_FreeSurface(boss_spr0);
    if(boss_spr1 != NULL)
        SDL_FreeSurface(boss_spr1);
    if(boss_spr2 != NULL)
        SDL_FreeSurface(boss_spr2);
    if(boss_spr3 != NULL)
        SDL_FreeSurface(boss_spr3);
